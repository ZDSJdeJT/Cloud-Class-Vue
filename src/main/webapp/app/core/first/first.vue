<template>
  <div>
    ...
    <a-button
      type="primary"
      @click="showModal"
      v-text="$t('first.releaseDynamic')"
      style="position: absolute; top: 1.3rem; left: 88%"
    >
    </a-button>
        ...
        <a-comment :author="item.petName">
          ...
          <template slot="actions">
            ...
            <span key="comment-basic-reply-to">
              <a-tooltip title="查看评论" placement="bottom"
                ><a-icon type="message" @click="viewComments(index)" /></a-tooltip
              >&nbsp;{{ item.commentsCount }}
            </span>
            <span v-if="getDynamicDeletePower(index)">&nbsp;&nbsp;&nbsp;<font color="red" @click="deleteDynamic(index)">删除</font></span>
          </template>
          <p slot="content">
            <a :id="spliceId('dynamicContent', index)" class="dynamicStyle"
              >{{ showPartOfDynamicContent(index) }}&nbsp;&nbsp;</a
            ><span key="view-dynamic-content-details">
              <a-tooltip
                placement="bottom"
                title="查看详情"
                v-if="isDynamicContentTooLong[index]"
                ><a-icon
                  type="plus-circle"
                  @click="viewDynamicContentDetails(index)" /></a-tooltip
              >&nbsp;<a-tooltip
                placement="right"
                title="收起详情"
                v-if="isDynamicContentTooLong[index]"
                ><a-icon
                  type="minus-circle"
                  @click="cancelViewDynamicContentDetails(index)"
              /></a-tooltip>
            </span>
          </p>
          ...
          <div :id="spliceId('dynamicComment', index)" hidden>
            <div>
              ...
              ></a-botton>
              <a-comment
                v-for="commentOfDynamic in dynamicCommentList[
                  getIndexOfDynamicCommentList(index)
                ]"
                :key="commentOfDynamic.id"
              >
                <span slot="actions">
                  <span key="comment-basic-like">
                    <a-tooltip title="点赞">
                    ...
                    <span style="padding-left: '8px'; cursor: 'auto'">
                      {{ commentOfDynamic.tagsCount }}&nbsp;&nbsp;&nbsp;&nbsp;
                    </span>
                  </span>
                  <span
                    slot="actions"
                    @click="viewReplay(commentOfDynamic.id, commentOfDynamic.id)"
                    >回复</span
                  >
                ...
                <a-avatar
                  slot="avatar"
                  :src="commentOfDynamic.postUserHeadPortraitUri"
                  alt="co"
                />
                <p slot="content">
                ...
                <a-comment v-for="reply in commentOfDynamic.comments" :key="reply.id">
                  <span slot="actions">
                    ...
                    <span
                      slot="actions"
                      @click="viewReplay(commentOfDynamic.id, reply.id)"
                      >回复</span
                    >
                    <span>&nbsp;&nbsp;&nbsp;<font color="red">删除</font></span>
                  </span>
                  ...
                  <a-avatar
                    slot="avatar"
                    :src="reply.postUserHeadPortraitUri"
                    alt="Han Solo"
                  />
                  <p slot="content">
                    <a class="dynamicStyle"
                      >@回复 {{ reply.replyPetName }}:{{ reply.content }}</a
                    >
                  </p>
                  <a-tooltip slot="datetime">
                    <span>时间</span>
                  ...
                  <a-textarea placeholder="发条友善的评论" class="space-align-container" align="center" allow-clear v-model="replayText"/>
                  <a-botton type="primary" class="space-align-block" align="end" shape="circle" icon="export" @click="replay(item.id,commentOfDynamic.id)"/>
                  </div>
              </a-comment>
            </div>
          </div>
